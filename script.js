...
resetBtn.addEventListener('click', () => {
    if (confirm('هل أنت متأكد من إعادة تعيين العدادات؟')) {
        count = 0;
        tasbeehCount = 0;
        tahmeedCount = 0;
        takbeerCount = 0;
        ...
        updateCounter();
        updateCounter('tasbeeh');
        updateCounter('tahmeed');
        updateCounter('takbeer');
        updateCounter('tahlil');
        updateCounter('istighfar');
        updateCounter('hawqala');
        
        // Shake all counters
        const counters = document.querySelectorAll('.individual-counter, #counter');
